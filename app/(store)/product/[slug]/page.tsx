    ...
    <div className="container mx-auto px-4 py-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className={`relative aspect-square overflow-hidden rounded-lg , shadow-lg ${isOutOfStock ? "opacity-50" : ""}`}>
          {product.image && <Image src={imageUrl(product.image).url()} alt={product.name ?? "Product Image"} fill className="object-contain transition-transform duration-300 hover:scale-105" />}
          {isOutOfStock && (
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <div className="text-xl font-semibold mb-4">IDR{product.price?.toFixed(2)}</div>
